...
class GaussianClassifierTiedDiagCov:
    ...
    def predictAndGetScores (self, X):
        #LS0 = multivariateGaussian.logpdf_GAU_ND(X, self.mean0, self.sigma )
        #LS1 = multivariateGaussian.logpdf_GAU_ND(X, self.mean1, self.sigma )
        LS0 = numpy.asarray(multivariateGaussian.logpdf_GAU_ND(X, self.mean0, self.sigma)).flatten()
        LS1 = numpy.asarray(multivariateGaussian.logpdf_GAU_ND(X, self.mean1, self.sigma)).flatten()
        #log-likelihood ratios
        llr = LS1-LS0
        return llr